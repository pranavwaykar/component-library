...
export const Default = {
  name: 'Default',
  render: (args) => {
    const [idx, setIdx] = React.useState(args.activeIndex ?? 0);
    React.useEffect(() => { if (typeof args.activeIndex === 'number') setIdx(args.activeIndex); }, [args.activeIndex]);
    return React.createElement(SideMenu, {
      ...args,
      activeIndex: idx,
      onMenuClick: (menu, index, event) => {
        setIdx(index);
        if (typeof args.onMenuClick === 'function') args.onMenuClick(menu, index, event);
      },
    });
  },
  args: {
